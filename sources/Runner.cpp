...
const char* ConvertType(int type)
{
  ...
  unsigned __int64 Massive::DirectPass(int* begin, int* end, int size, int count)
{
 ...
 unsigned __int64 Massive::BackPass(int* begin, int* end, int size, int count)
{
    std::chrono::time_point<std::chrono::system_clock> start, stop;
    unsigned __int64 avg = 0;
    for (int k = 0; k < count; k++){
...
unsigned __int64 Massive::RandomPass(int* begin, int* end, int size, int count)
{
...
void Massive::StartLoop(double size, int count, int type)
{
    srand(time(0));
